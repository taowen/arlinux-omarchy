...
exec "$files/bin/bionicx-exec" --wait-primary --cwd "$root" \
  --env "BIONICX_ROOTFS=$root" --env "BIONICX_FILES=$files" \
  --env "LD_LIBRARY_PATH=$root/usr/lib/mesa:$root/usr/lib/arlinux-platform" -- \
